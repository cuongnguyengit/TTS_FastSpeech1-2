...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        D_path = os.path.join(
            hparams.data_path, 'outdir', "alignment", "{}-ali-{}.npy".format(hparams.dataset, basename))
        D = np.load(D_path)
        f0_path = os.path.join(
            hparams.data_path, 'outdir', "f0", "{}-f0-{}.npy".format(hparams.dataset, basename))
        f0 = np.load(f0_path)
        energy_path = os.path.join(
            hparams.data_path, 'outdir', "energy", "{}-energy-{}.npy".format(hparams.dataset, basename))
